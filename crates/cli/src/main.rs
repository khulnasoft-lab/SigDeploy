...
#[derive(Parser)]
#[clap(name = "sigdeploy", global_setting(clap::AppSettings::NoAutoVersion))]
struct Args {
    ...
    #[clap()]
    paths: Vec<PathBuf>,
    /// Print Sigdeploy's version and the app path.
    #[clap(short, long)]
    version: bool,
    /// Custom Sigdeploy.app path
    #[clap(short, long)]
...
fn launch_app(app_path: PathBuf) -> Result<(IpcSender<CliRequest>, IpcReceiver<CliResponse>)> {
    ...
    let url = format!("sigdeploy-cli://{server_name}");
